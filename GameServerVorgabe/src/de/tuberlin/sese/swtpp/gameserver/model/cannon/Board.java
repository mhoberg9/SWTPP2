...
public class Board {
	 ...
	 * @TODO !!!!!!!!!!!!!!!!!!!!!!!
	 * 
	 *       algorithmus muss in anyMatch Stream geschehen und f�r jedes Element,
	 *       was zum spieler geh�rt anwenden
	 * 
	...
	public boolean canStillPlay(String requestingPlayer) {
		return fieldList.stream().anyMatch(a -> {
			...
			if (inDanger(posFields, requestingPlayer) && !retreatFields(posFields).contains("NO RETREAT")
					...
					|| !hasNeighbor(posFields, requestingPlayer).contains("NO NEIGHBOR"))
				return true;
			return false;
		});
	}
